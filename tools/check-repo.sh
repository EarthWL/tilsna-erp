...
# ตรรกะ + ค่าคาลิเบรตอยู่ใน tools/sizecheck.sh ที่เดียว (preflight.sh ก็เรียกตัวเดียวกัน)
if [ -f tools/sizecheck.sh ]; then
  # shellcheck source=/dev/null
  . tools/sizecheck.sh && run_sizecheck "  " || true
  say y "tools/sizecheck.sh รันได้"
else
