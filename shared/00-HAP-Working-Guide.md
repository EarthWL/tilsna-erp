...
`[V]` 🔴🔴 **`≠` ประเมินเป็น FALSE เมื่อฟิลด์เป็นค่าว่าง — เฉพาะใน `operateCondition` ของ branch เท่านั้น ไม่ใช่ใน `filters`** · **branch:** พิสูจน์ด้วย A/B controlled test บน WF-HR-02 (record เหมือนกันทุกฟิลด์ ต่างแค่ flag `""` vs `"0"`) — ตัวที่ `"0"` ทำงานครบสาย ตัวที่ `""` ตกไป catch-all **เงียบสนิท ไม่มี error ไม่มีร่องรอยบน record** · **`filters` ของ get_multiple/search: ตรงข้ามกัน — ค่าว่างผ่านตามปกติ** พิสูจน์ 31 ส.ค. ด้วย probe workflow ที่คุมตัวแปรครบ (ฟิลด์ Number ชนิดเดียวกัน · `conditionId 10` ตัวเดียวกัน · `ignoreEmpty 0` เท่ากัน) ได้ 2 แถว = แถวที่ค่า `"0"` **และแถวที่ค่าว่าง** พร้อม control ยืนยันว่า filter ไม่ได้ถูกเมิน ⇒ 🔴 **ห้ามเหมาว่าอาการที่ branch เกิดที่ filter ด้วย** — เราเคยเหมาแบบนั้นแล้วไปแก้จุดที่ไม่ได้พัง 3 จุดสองโมดูล · ทางแก้ที่ถูกสำหรับ **branch**: ใช้ `flag eq "1"` → เส้น no-op แล้วให้ catch-all เป็นเส้นทำงาน หรือเพิ่ม OR group `is empty` (`conditionId 8`)
...
| `8` | ว่าง (is empty) | ✅ `[V]` **ข้ามโมดูล + ข้าม context** | HR branch (ยิงจริง) · บัญชี branch (`ZZTEST-REV-001` 31 ส.ค.) · **`filters` ของ get_multiple (probe 31 ส.ค. — กลุ่ม `="77"` OR `8` ได้ 1 แถว ขณะที่ `="77"` เดี่ยวได้ 0)** ⇒ ใช้ได้ทั้งสอง context |
| `9` | เท่ากับ (=) | ✅ `[V]` | บัญชีใช้ 18 จุด (มากที่สุด) |
| `10` | ไม่เท่ากับ (≠) **สำหรับฟิลด์ Number** | ✅ `[V]` | **ตัวที่ก่อ D-19 — แต่เฉพาะใน branch** · ใน `filters` ทำงานถูกต้อง (ค่าว่างผ่าน) · บัญชีใช้ 8 จุด · HR 5 จุด |
...
| **`1`** | ❓ ไม่รู้ | 🟡 | บัญชี 6 จุด (`Record ID`, `เดือนภาษี`) |
| **`29`** | ❓ ไม่รู้ | 🟡 | บัญชี 4 จุด (relation, `isCheckDay`) — ใช้คัดบัญชีตอนปิดปี |
| **`2`** | **ไม่เท่ากับ (≠) สำหรับฟิลด์ Text** | ✅ `[V]` **31 ส.ค.** | probe: `flag ≠ "1"` บนฟิลด์ Text ได้ 2 แถว (ถ้าเป็น "=" จะได้ 6) ⇒ **`conditionId` ของ operator เดียวกันต่างกันตามชนิดฟิลด์: `≠` = `10` บน Number แต่ = `2` บน Text** · MCP `batch_create_process_nodes` แปลง `op:"ne"` เป็นเลขไหน **ขึ้นกับชนิดฟิลด์** ตามที่ tool description เขียนไว้จริง |
| **`42`** | ❓ ไม่รู้ | 🟡 | 🆕 **HR ก็ใช้** — WF-HR-05 filter ฟิลด์วันที่ `วันที่ทำงาน` โดย `conditionValues` **ว่าง** (`[None]`) ⇒ น่าจะเป็น operator วันที่แบบไม่ต้องมีค่า (เช่น "วันนี้"/"ก่อนวันนี้") · อยู่ตระกูลเดียวกับ `41` ที่บัญชีใช้ 4 จุดบนฟิลด์วันที่ |
...
🔴 **ช่องว่างที่ยังเปิดอยู่ทั้งสองโมดูล:** `conditionId 8` **ใน `filters` ของ search(7)/get_multiple(13)** ยังไม่มีใครพิสูจน์ — กระทบ WF-HR-05 (HR) และ WF-AC-09/WF-AC-12 (บัญชี) · จน��ว่าจะพิสูจน์ได้ **ห้ามนับจุดเหล่านั้นว่า "แก้แล้ว"**
