...
### 15.5 🔴🔴 `checkType: 1` บล็อกการเขียนผ่าน API/CLI **เฉพาะเมื่อ payload แตะฟิลด์ที่กฎอ้างถึง** — แก้ 7 ก.ย. 2569

> ⚠️ **ข้อความเดิมของหัวข้อนี้ผิด** เคยสรุปว่า "ไม่บล็อก API เลย" เพราะทดสอบด้วยการเขียนฟิลด์ที่**ไม่ได้อยู่ในกฎ** แล้วเห็นว่าผ่าน · ทดสอบใหม่แบบมีตัวควบคุมได้ผลชัดกว่านั้นมาก

ยิงจริง 3 เคส ขณะที่ BR-19.1 (`Rollup > วงเงินคงเหลือขณะยื่น`) **ถูกละเมิดอยู่**:

| เคส | payload | ผล |
|---|---|---|
| A | ฟิลด์ที่กฎ**ไม่ได้**อ้างถึง (`เหตุผลที่ไม่อนุมัติ`) | ✅ `resultCode 1` ผ่าน |
| B | ฟิลด์ที่กฎอ้างถึง ด้วยค่าที่**ละเมิด**กฎ | 🔴 **`resultCode 32` ถูกปฏิเสธ** |
| C | ฟิลด์เดียวกัน ด้วยค่าที่**ไม่ละเมิด** | ✅ ผ่าน |

⇒ เอนจินตรวจ **เฉพาะกฎที่มีฟิลด์ปรากฏใน payload** ไม่ได้ตรวจทุกกฎทุกครั้ง

🔑 **ข้อความ error บอก ruleId มาให้ด้วย** — `It rejected these fields: <ruleId>:<rowid>` · เอา id ไปเทียบกับ `worksheet rules` ได้เลยว่าโดนกฎไหน (ที่เห็นเป็น "fields" จริง ๆ คือ ruleId ไม่ใช่ controlId)

🔴 **workflow ยังข้ามกฎได้จริง** — WF-HR-11 เขียน `วงเงินคงเหลือขณะยื่น = −2,600` ลงไปได้ทั้งที่ละเมิด BR-19.1 · ตรงกับ §2 ข้อ 29 (workflow ข้าม `required`) ⇒ **กฎฟอร์มกันคนกรอกและกัน API ได้บางส่วน แต่กัน workflow ไม่ได้เลย**

🔴🔴 **กับดักออกแบบที่เจอจากเรื่องนี้: กฎที่เทียบกับยอดคงเหลือจะ "ย้อนกัดตัวเอง" หลัง workflow ตัดยอด**
BR-19.1 เขียนว่า "ยอดขอเบิก > วงเงินคงเหลือ ⇒ บล็อก" ซึ่งถูกต้อง **ก่อน** อนุมัติ · แต่หลัง WF-HR-11 ตัดวงเงินแล้ว snapshot ลดลงจนต่ำกว่ายอดที่เบิกเป็นเรื่องปกติ ⇒ กฎกลายเป็นจริงตลอดไป และไปบล็อกการแก้ record นั้นผ่าน API
**ทางแก้:** ใส่เงื่อนไขสถานะกำกับเสมอ (`สถานะ ≠ Approved`) ให้กฎมีผลเฉพาะช่วงที่ยังแก้ได้จริง
...
## 17. `showControls` — คอลัมน์ที่ relation แสดง · และวิธีเขียน `update-fields` ให้ตารางใหญ่ผ่าน · 6 ก.ย. 2569

### 17.1 ไม่ตั้ง `showControls` = ปล่อยให้ระบบเดา แล้วมันเดาแย่

relation field ที่ `showControls` เป็น `[]` จะแสดง **3 ฟิลด์แรกตามลำดับ layout ของตารางปลายทาง** ⇒ ตารางพนักงานได้ `รหัสพนักงาน · คำนำหน้า · ชื่อ (ไทย)` — เสีย 1 ใน 3 คอลัมน์ให้ "นาย/นางสาว" และไม่เห็นนามสกุลหรือตำแหน่ง

กระทบ **ทุกที่ที่ relation โผล่**: แท็บ record ที่เกี่ยวข้องท้ายฟอร์ม · ตารางฝังในฟอร์ม (`showtype 5`) · การ์ด/ดรอปดาวน์ตอนเลือก record

⇒ **ตั้ง `showControls` ทุก relation field เสมอ** และตั้งเป็น "โปรไฟล์ต่อตารางปลายทาง" (ตารางเดียวกันถูกอ้างจากที่ไหนก็แสดงคอลัมน์ชุดเดียวกัน) ไม่ใช่ตั้งทีละจุดตามอารมณ์

**สูตรเลือกคอลัมน์ 3–6 ช่อง:** `รหัส/เลขที่` → `ชื่อที่คนอ่านออก` → `ตัวจำแนก 1–2 ตัว (ประเภท/สถานะ)` → `ตัวเลขที่ต้องเห็น (ยอด/จำนวน)` · หลีกเลี่ยงคำนำหน้า ธง `(ระบบ)` ฟิลด์ยาว ๆ และ relation ซ้อน relation

### 17.2 🔴 `update-fields` payload ใหญ่เกิน `ARG_MAX` — ตัด `relationControls` ทิ้งได้

`worksheet fields --raw` คืน `relationControls` (สำเนาฟิลด์ทั้งชุดของตารางปลายทาง) มาในทุก relation field ⇒ ตารางที่มี relation เยอะบวมมาก · `hr_employee` = **585 KB** ⇒ ส่งเข้า `--controls` แล้วได้ `OSError: [Errno 7] Argument list too long` (ลิมิตอาร์กิวเมนต์เดียวของ Linux ~128 KB)

**ทางแก้ที่พิสูจน์แล้ว:** ลบคีย์ `relationControls` ออกจากทุก control ก่อนส่ง — **เซิร์ฟเวอร์เติมกลับให้เองครบถ้วน**

```python
for c in ctl: c.pop('relationControls', None)
```

ทดสอบยืนยัน: อ่านกลับหลังเขียน → จำนวน `relationControls` ของทุก field เท่าเดิม · `showControls`/`dataSource`/`required` ไม่เปลี่ยน · `hr_employee` 585 KB → **95 KB** ผ่านฉลุย

⚠️ `hap` CLI **ไม่รองรับ `--controls @file.json`** (ตอบ `JSONDecodeError`) — ต้องลดขนาด payload เอง

### 17.3 กวาดทั้งโมดูลแล้วต้องนับกลับ

การกวาดครั้งนี้เขียน `update-fields` ทับ **34 ตาราง** ⇒ ตรวจหลังยิงด้วยสองเกณฑ์เสมอ: (ก) **จำนวนฟิลด์ของทุกตารางต้องเท่าเดิม** (จับ field หายจากการเขียนทับ) (ข) **`showControls` ของทุก relation field ต้องตรงโปรไฟล์** · ผลรอบนี้ 115/115 ตรง · 0 ตารางฟิลด์หาย

---

---

## 18. จุดบอดของการออดิตภาษา: `hint` (placeholder) — 6 ก.ย. 2569

การออดิตภาษาจีนรอบก่อนตรวจ **ชื่อฟิลด์ · ชื่อ view · Record Name · รูปแบบวันที่** แต่**ไม่ได้ตรวจ `hint`** ⇒ ตารางที่สร้างใหม่หลังจากนั้นยังมีข้อความจีนโผล่ในฟอร์มเป็น placeholder จาง ๆ ในช่องกรอก

`hap worksheet add-fields` ที่**ไม่ได้ส่ง `hint`** มาเลย เซิร์ฟเวอร์จะเติม default ภาษาจีนให้เองตามชนิดฟิลด์:

| ชนิดฟิลด์ | `hint` ที่ระบบเติมให้ | ควรเป็น |
|---|---|---|
| 6 Number | `请填写数值` | `""` หรือข้อความไทยสั้น ๆ |
| 15 Date · 16 DateTime | `请选择日期` | `เลือกวันที่` |
| 9/11 Select | `请选择` | `เลือกรายการ` |
| 26 Collaborator | `请选择成员` | `เลือกผู้ใช้ในระบบ` |
| 14 Attachment | `添加附件` | `แนบไฟล์` |

⇒ **ส่ง `hint` เองทุกครั้งที่สร้างฟิลด์** (ใส่ `""` ก็ได้ ขอให้ส่ง) · และเวลาออดิตภาษา **ต้องกวาด `hint` ด้วย ไม่ใช่แค่ `controlName`**

```python
cjk = re.compile(r'[\u4e00-\u9fff]')
bad = [c for c in ctl if cjk.search((c.get('controlName') or '') + (c.get('hint') or ''))]
```

🔎 อีกจุดที่พลาดง่ายในชุดเดียวกัน: **ฟิลด์ relation ที่ระบบตั้งชื่อจาก alias ดิบ** (เจอ 2 จุดชื่อ `hr_salary_structure` โผล่เป็น label บนฟอร์ม) — เกณฑ์ตรวจคือ "ชื่อฟิลด์ที่ไม่มีอักษรไทยเลย" ไม่ใช่แค่ "มีอักษรจีน"

---

---

## 19. 🔎 ตรวจสอบ D-59 ของฝั่งบัญชี: `triggerFields` **จำกัดการยิงจริง** (ฝั่ง HR) — 7 ก.ย. 2569

`handoff/AC-WF-BATCH.md` แจ้ง **D-59: "`triggerFields` ไม่ได้จำกัดให้ยิงเฉพาะตอนฟิลด์นั้นเปลี่ยน — update ฟิลด์ไหนก็ยิง"** · ทดสอบซ้ำฝั่ง HR แล้ว **ไม่เกิดตามนั้น**

**การทดลอง** — WF-HR-11 (`worksheet_event` · `event: update` · `triggerFields: [สถานะใบเบิก]`) บน record ที่สถานะ = Approved

| ขั้น | ทำอะไร | ผล |
|---|---|---|
| negative control | update **เฉพาะ** ฟิลด์ธง (ไม่อยู่ใน `triggerFields`) โดยตั้งธง = 0 | **workflow ไม่ยิง** — `record logs` มีแค่บรรทัดที่เราเขียนเอง ไม่มีรายการของ `user-workflow` ตามมา · วงเงินไม่ขยับ |
| positive control | update `สถานะใบเบิก` (อยู่ใน `triggerFields`) ขณะธง = 0 | **ยิงทันที** — ตัดวงเงินซ้ำจริง (2,800 → 5,600 · คงเหลือ 200 → −2,600) |

⇒ สรุปเท่าที่พิสูจน์ได้: **`triggerFields` ทำงานตามที่โฆษณาสำหรับ `worksheet_event` + `event: update`** · D-59 อาจเกิดจาก config ที่ต่างออกไป (เช่น `add_or_update` หรือ workflow ที่ไม่ได้ตั้ง `triggerFields` เลย) — **ยังไม่ได้ทดสอบสองกรณีนั้น**

🔴 **แต่ข้อสรุปเชิงปฏิบัติของ agent-ac ยังถูกต้อง**: positive control ข้างบนพิสูจน์ว่า **ธงกันซ้ำคือเกราะชั้นเดียวจริง ๆ** — พอธงถูกรีเซ็ตด้วยเหตุใดก็ตาม ยอดถูกตัดซ้ำทันทีโดยไม่มีอะไรทัดทาน ⇒ **ยังควรทำให้ idempotent ถ้าทำได้ และต้องทดสอบยิงซ้ำทุกครั้ง**

**สถานะเกราะของ workflow ฝั่ง HR ที่เขียนยอดแบบบวก/ลบทับของเดิม (ตรวจ 7 ก.ย. 2569)**

| workflow | สูตร | gate |
|---|---|---|
| WF-HR-02 ตัดสิทธิลา | `ใช้ไปแล้ว + จำนวนวันลา` | ✅ `[[สถานะ=Approved, ธง≠1],[สถานะ=Approved, ธงว่าง]]` |
| WF-HR-03 คืนสิทธิลา | `ใช้ไปแล้ว − จำนวนวันลา` | ✅ `[[Cancelled, เคยตัด, ธงคืน≠1],[Cancelled, เคยตัด, ธงคืนว่าง]]` |
| WF-HR-11 ตัดวงเงิน | `วงเงินที่ใช้ไป + ยอดรวมบรรทัด` | ✅ `[[Approved, ธง≠1],[Approved, ธงว่าง]]` |

ทั้งสามใช้รูปแบบกระจาย OR-of-AND ที่มีสาขา "ธงว่าง" ครบตามบทเรียน D-19 · ⬜ **WF-HR-03 ยังไม่เคยทดสอบยิงซ้ำจริง** (อีกสองตัวทดสอบแล้วผ่าน)
