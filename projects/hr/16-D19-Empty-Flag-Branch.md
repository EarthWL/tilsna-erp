...
| **RUN-D1** | `flag ≠ "1"` บนฟิลด์ **Text** (`conditionId 2`) | **2** | ถ้า `2` เป็น "=" จะได้ 6 · ได้ 2 ⇒ ✅ **`2` = ไม่เท่ากับ (สำหรับ Text)** และค่าว่างผ่านอีกเช่นกัน |
...
**(2) 🔴🔴 D-19 เป็นบั๊กของ `operateCondition` (branch) เท่านั้น — `filters` ของ get_multiple/search ไม่ได้เป็นบั๊ก**
...
- ใน **filters** → ค่าว่าง **ผ่าน** (พิสูจน์ 31 ส.ค. ด้วย RUN-A + control RUN-C2)
...
| "WF-AC-09/12 มี 2 จุด get_multiple ที่เสี่ยง D-19" | **ทั้งสองจุดไม่เคยเป็นบั๊ก** — แจ้ง agent-ac แล้วใน `handoff/HR-COND8-PROOF.md` |
