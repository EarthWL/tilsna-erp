...
### WF-AC-24 กันใบแจ้งหนี้ซ้ำ (BR-22 / TC-09) — **สร้างและ publish แล้ว** `6a96a92fe6605c4b130f949b` ✅ 1 ก.ย. 2569 · **publishVersion 3** (v3 ปิด R6 + R7)
...
#### Node ที่สร้างจริง (โครงรอบ v3 — v1/v2 ถูกลบสร้างใหม่ทั้งกลุ่ม)
...
| 5 | `b_dup` | `6a96acad0e97bf440d0fc042` | branch condition | `p_isdup`: `c_stale›needBlock` > 0 · `p_pass`: fallback |
| 6 | `rej_dup` | `6a96acad0e97bf440d0fc043` | update_record | สถานะ → ร่าง · `submitted_flag` → 0 · เขียน `dup_key` = `c_stale›curKey` · เขียน `reject_reason` |
| 7 | `notify_dup` | `6a96acad0e97bf440d0fc044` | send_internal_notice | แจ้งผู้สร้างเอกสาร (`trigger›caid`) |
| 8 | `pass_ok` | `6a96acad0e97bf440d0fc045` | update_record | **บนสาย `p_pass`** — ล้าง `reject_reason` · `submitted_flag` → **1** · เขียน `dup_key` = `c_stale›curKey` |
...
### WF-AC-25 ออกใบสำคัญตั้งหนี้จากใบตั้งหนี้ (งาน 3.5 ครึ่งหลัง) — **สร้างและ publish แล้ว** `6a970bc77dc381d682a550ba` ✅ 2 ก.ย. 2569 · publishVersion 2 · สายลูก `6a970c20500f6714e479dfe8` (publish v2)
...
| R13 | **ไม่มีการตรวจว่างวดบัญชียังเปิดอยู่ก่อนออกใบสำคัญ** | WF-AC-01 ตรวจให้ แต่เส้นทางนี้ไม่ผ่าน WF-AC-01 ⇒ ถ้างวดปิดไปแล้วจะยังออกใบสำคัญได้ · **ควรเพิ่มก่อนใช้จริง** |
