...
### WF-AC-01 อนุมัติใบสำคัญ — **สร้างและ publish แล้ว** `6a8eaa45e6605c4b13ccf49b` · **publishVersion 3 (1 ก.ย. 2569 — แก้ D-34)**
...
| 2 | `set_flag` | update_record | `submitted_flag`=1 · `approval_level`=1 · **`reject_reason` → ล้างค่า (เพิ่ม 1 ก.ย. 2569 — nodeId ใหม่ `6a96ae8e5f8564a68c6d4f90`)** |

> 🔴 **จุดที่ต้องแก้ในข้อ 2 — เจอจากการไล่ตรวจตามบทเรียน D-34 ไม่ได้เจอจากการใช้งาน**
> `rej_unbal` และ `rej_period` เขียน `biz_vch_reject_reason` แต่ **ไม่มี node ไหนในทั้ง workflow ล้างมันเลย** ⇒ ใบที่ถูกตีกลับ แก้จนถูก แล้วส่งใหม่ จะยัง **ค้างข้อความตีกลับของรอบก่อน** ทั้งที่ผ่านแล้ว
> **วิธีแก้:** ให้ `set_flag` (ซึ่งวิ่งทุกครั้งที่เริ่มรอบส่งอนุมัติใหม่) ล้าง `reject_reason` ไปด้วย — ดีกว่าไปเพิ่ม node บนสาย `p_open` เพราะครอบคลุมทุกทางที่เข้ามารอบใหม่
> ⚠️ **วิธีทำ:** MCP แก้ node ไม่ได้ ⇒ `delete_process_node` แล้วสร้างใหม่ · **หลังสร้างต้องอ่าน `prevNode` กลับมาดูว่าต่ออยู่ระหว่าง `p_go` กับ `chk_balance` จริง** ก่อน publish (ยืนยันแล้วรอบนี้)
...
### WF-AC-24 ด่านตรวจตอนส่งใบตั้งหนี้เพื่ออนุมัติ (BR-22 กันใบซ้ำ + BR-03 กันงวดปิด) — **สร้างและ publish แล้ว** `6a96a92fe6605c4b130f949b` ✅ 1–2 ก.ย. 2569 · **publishVersion 6** (v3 ปิด R6 + R7 · **v6 ปิด R13**)
...
#### Node ที่สร้างจริง (โครงรอบ v6 — v4/v5 ถูกลบสร้างใหม่ทั้งกลุ่ม ดูบทเรียนท้ายหัวข้อ)
...
| 2 | `get_dups` | `6a96a964e6605c4b130f9691` | get_multiple | `AC_AP` filter `invoice_no` eq trigger · `partner` eq trigger · `rowid` **ne** trigger · limit 50 |
| 3 | `cnt_dups` | `6a96a964e6605c4b130f9692` | rollup count | target = `get_dups` |
| 4 | `c_stale` | `6a96acad0e97bf440d0fc041` | **code (javascript)** | คิดคีย์ปัจจุบัน `คู่ค้า\|เลขที่ใบ` · ตัดสิน `needBlock` = (ซ้ำ>0 และเหตุผลใช้ไม่ได้) · output: `needBlock` `curKey` `dupCount` `trace` |
| 5 | `get_period` | `6a97108817431051c38ac40c` | get_single | ดึงงวดบัญชีของเอกสารจาก `AC_PERIOD` (filter `rowid` eq `trigger›biz_ap_period`) · `ifEmpty` = **continue** (ตั้งใจ — ต้องไปตกสายตีกลับ ไม่ใช่หยุดเงียบ) |
| 6 | `b_check` | `6a9712a9aca53a7728675c53` | branch condition (firstMatch, 3 สาย) | `p_pass`: `get_period›period_status` **eq เปิด** `f662571c-…` **และ** `c_stale›needBlock` < 1 · `p_isdup`: งวดเปิด **และ** `needBlock` > 0 · `p_closed`: **fallback** |
| 7 | `pass_ok` | `6a9712a9aca53a7728675c54` | update_record | **บนสาย `p_pass`** — ล้าง `reject_reason` · `submitted_flag` → **1** · เขียน `dup_key` = `c_stale›curKey` |
| 8 | `rej_dup` | `6a971333aca53a772867619b` | update_record | **บนสาย `p_isdup`** — สถานะ → ร่าง · `submitted_flag` → 0 · เขียน `dup_key` · เขียน `reject_reason` (BR-22) |
| 9 | `notify_dup` | `6a971333aca53a772867619c` | send_internal_notice | แจ้งผู้สร้างเอกสาร (`trigger›caid`) |
| 10 | `rej_closed` | `6a97134eaca53a772867629c` | update_record | **บนสาย `p_closed`** — สถานะ → ร่าง · `submitted_flag` → 0 · เขียน `dup_key` · เขียน `reject_reason` (BR-03 งวดปิด) |
| 11 | `notify_closed` | `6a97134eaca53a772867629d` | send_internal_notice | แจ้งผู้สร้างเอกสาร |

**เกณฑ์ตัดสินใน `c_stale` (ตรรกะทั้งหมดอยู่ที่เดียว)**

```
justValid = (เหตุผลไม่ว่าง) และ (dup_key เดิมว่าง  หรือ  dup_key เดิม == คีย์ปัจจุบัน)
needBlock = (จำนวนใบซ้ำ > 0) และ (ไม่ justValid)
```

> 🔑 **ทำไมต้องเทียบ `dup_key` เดิมกับคีย์ปัจจุบัน — นี่คือวิธีปิด R7**
> `dup_key` ถูกเขียนใหม่ทุกครั้งที่จบรอบ (ทั้งสาย ตีกลับ และ ผ่าน) ⇒ มันคือ "คีย์ที่ระบบตรวจไว้ล่าสุด"
> ถ้าผู้ใช้แก้เลขที่ใบหลังจากกรอกเหตุผลไว้แล้ว คีย์จะไม่ตรง ⇒ เหตุผลเดิมถือว่าใช้ไม่ได้กับเลขใหม่ ⇒ ตีกลับให้ทบทวน
> **เงื่อนไข `dup_key เดิมว่าง` ต้องมี** ไม่งั้นใบที่กรอกเหตุผลมาแต่แรก (ยังไม่เคยผ่านการตรวจ) จะโดนตีกลับหนึ่งรอบโดยไม่จำเป็น
> 🔴 และเป็นเหตุผลที่ต้อง **ย้ายการเขียน `dup_key` ออกจากหัวสาย** — v1 มี node `set_key` เขียน `dup_key` ก่อนถึง branch ⇒ ค่าเดิมถูกทับไปแล้วตั้งแต่ยังไม่ได้เทียบ เทียบยังไงก็ตรงเสมอ
...
#### ✅ ผลทดสอบจริง 1 ก.ย. 2569 — ผ่านครบ 6 เคส

เอกสารทดสอบหลัก `92b57738-ebbd-4fbd-b2b2-d58091d4bcf2` (สร้างให้ซ้ำโดยตั้งใจ) · เคสสุดท้ายใช้ `ZZTEST-AP-GROSSUP-003` ซึ่งเลขไม่ซ้ำใคร

| เคส | ทำอะไร | ผล |
|---|---|---|
| **TC-09** | ส่งอนุมัติทั้งที่เลขซ้ำและยังไม่กรอกเหตุผล | ✅ ตีกลับเป็น **ร่าง** · `submitted_flag` = 0 · `reject_reason` = "ตีกลับอัตโนมัติ (BR-22): … ซ้ำกันอีก 1 ใบ …" · `dup_key` = `BTH-0006\|ZZTEST-AP-EXCL-001` · `_updatedBy` = `user-workflow` |
| **TC-09b** | กรอก `duplicate_justification` แล้วส่งใหม่ | ✅ ผ่านด่าน สถานะคง "รออนุมัติ" |
| **TC-09c** | ส่งซ้ำอีกรอบ (หลังเพิ่ม node ล้างค่า) | ✅ ผ่านด่าน **และ `reject_reason` ถูกล้าง** |
| **TC-09d** 🆕 | **แก้ `invoice_no` ไปชนใบอื่น ทั้งที่เหตุผลเดิมยังอยู่** | ✅ **ถูกตีกลับ** — เหตุผลที่กรอกไว้กับเลขเดิมใช้กับเลขใหม่ไม่ได้ · `dup_key` ถูกอัปเดตเป็นคีย์ใหม่ · **นี่คือหลักฐานว่า R7 ปิดแล้ว** |
| **TC-09e** 🆕 | ส่งอนุมัติอีกครั้งโดยไม่แก้อะไร (คีย์ตรงกับที่ตรวจไว้แล้ว) | ✅ ผ่านด่าน · `reject_reason` ว่าง · **`submitted_flag` = 1** ⇒ **R6 ปิดแล้ว** |
| **TC-09f** 🆕 | ใบที่เลขไม่ซ้ำใคร (`ZZTEST-AP-GROSSUP-003`) | ✅ ผ่านด่าน · `submitted_flag` = 1 · `dup_key` เขียนถูก · **ยอดของ WF-AC-23 ไม่เปลี่ยน** (10,000 / 0.00 / 700 / 309.28 / 10,700 / 10,390.72) ⇒ ยืนยันว่าสอง workflow บน trigger field เดียวกันอยู่ร่วมกันได้จริง |

#### ⚠️ ความเสี่ยงที่ยังไม่ได้แก้

| # | เรื่อง | ทำไมสำคัญ |
|---|---|---|
| ~~R6~~ ✅ **ปิดแล้ว 1 ก.ย. 2569** | `biz_ap_submitted_flag` เคยไม่มี workflow ไหนตั้งเป็น 1 (มีแต่ตัวที่ตั้ง 0) | แก้โดยให้ node `pass_ok` บนสาย `p_pass` ตั้งเป็น 1 — สายที่ผ่านด่านคือ "ส่งอนุมัติสำเร็จ" จริง ๆ · ยืนยันด้วย TC-09e / TC-09f |
| ~~R7~~ ✅ **ปิดแล้ว 1 ก.ย. 2569** | เหตุผลกำกับเคยใช้ได้ตลอดไปแม้ผู้ใช้แก้ `invoice_no` ไปชนใบอื่น | แก้โดย **ผูกเหตุผลกับคีย์** แทนการล้างฟิลด์ — `c_stale` เทียบ `dup_key` ที่ระบบตรวจไว้ล่าสุดกับคีย์ปัจจุบัน ถ้าไม่ตรงถือว่าเหตุผลหมดอายุ · **ดีกว่าการล้างฟิลด์ เพราะไม่ทำลายสิ่งที่ผู้ใช้พิมพ์ไว้ แค่บังคับให้ทบทวน** · ยืนยันด้วย TC-09d |
...
#### 🆕 ด่านงวดบัญชี (BR-03) — เพิ่มใน v4–v6 เพื่อปิด R13

**ทำไมมาไว้ที่ WF-AC-24 ไม่ใช่ WF-AC-25** — R13 บอกว่า "ออกใบสำคัญได้ทั้งที่งวดปิด" แต่การไปสอดด่านกลางสาย WF-AC-25 แปลว่าต้องรื้อ node ปลายน้ำ ~22 ตัวสร้างใหม่ (branch ไม่มีจุดบรรจบ) · WF-AC-24 เป็น **ด่านตอนส่งอนุมัติ** อยู่แล้ว มีทั้งทางตีกลับและทางแจ้งเตือนพร้อม และตรงกับวิธีที่ WF-AC-01 ตรวจงวดตอนส่งใบสำคัญ ⇒ ต้นทุนต่ำกว่ามากและได้ความหมายที่ถูกกว่า (ห้ามตั้งแต่ยื่นขออนุมัติ ไม่ใช่ปล่อยให้อนุมัติแล้วค่อยเงียบ)
**และยังใส่กันชนชั้นสองไว้ที่ WF-AC-25 ด้วย** (`chk_period`) เผื่อมีใครเขียนสถานะ "รับรู้แล้ว" ตรง ๆ โดยไม่ผ่านด่านนี้ — ดูหัวข้อ WF-AC-25

🔴 **ทำไมต้องเขียนเป็น "งวดเปิด" (บวก) แทน "งวดไม่เปิด" (ลบ)**
รอบแรก (v4) เขียนสาย `p_closed` เป็น `period_status is empty` **OR** `period_status ne เปิด` ตามสูตรแก้ D-19 — **ผลคือไม่ทำงาน** เอกสารที่งวดปิดกลับไหลไปสาย fallback (ผ่าน) · สรุปว่า **OR group ในตัวกรองของ branch path ไม่ถูกใช้จริงตอน runtime** (ยุบเป็น AND) ⇒ `empty AND ne` เป็นเท็จเสมอเมื่อฟิลด์มีค่า
⇒ v6 พลิกเป็นตรรกะบวกทั้งหมด: สายที่ "ผ่าน" ต้องพิสูจน์ว่า **งวด = เปิด** และให้ **สาย fallback เป็นสายตีกลับ** ⇒ ทุกกรณีที่พิสูจน์ไม่ได้ (งวดปิด · ไม่ได้ระบุงวด · หา record งวดไม่เจอ) ตกไปที่ตีกลับโดยอัตโนมัติ — **ปลอดภัยโดยโครงสร้าง ไม่ต้องพึ่ง OR** (บันทึกเป็น D-43)

#### ✅ ผลทดสอบด่านงวดบัญชี 2 ก.ย. 2569 — ผ่านครบ 3 เคส

ใช้งวดทดสอบ `ZZTEST-งวดปิด-R13` (สถานะ "ปิดชั่วคราว") และใบตั้งหนี้ `ZZTEST-R13/0001` ของ `SUP-0007` · **ลบทั้งคู่ทิ้งแล้วหลังทดสอบ (soft delete กู้คืนได้)**

| เคส | ทำอะไร | ผล |
|---|---|---|
| **TC-R13-a** | ผูกใบกับงวดที่ปิด แล้วส่งอนุมัติ | ✅ ตีกลับเป็น **ร่าง** · `submitted_flag` = 0 · `reject_reason` = "ตีกลับอัตโนมัติ (BR-03): งวดบัญชีของเอกสารนี้ปิดแล้ว …" · `_updatedBy` = `user-workflow` |
| **TC-R13-b** | ย้ายใบเดียวกันไปงวด `2569-12` (เปิด) แล้วส่งอนุมัติ | ✅ ผ่านด่าน · สถานะคง "รออนุมัติ" · `submitted_flag` = 1 · `reject_reason` ถูกล้าง ⇒ **ไม่กระทบเส้นทางปกติ** |
| **TC-R13-c** | ย้ายกลับงวดปิด แล้วเขียนสถานะ "รับรู้แล้ว" ตรง ๆ ข้ามด่าน | ✅ **ไม่มีใบสำคัญเกิดขึ้นเลย** — `chk_period` ใน WF-AC-25 หยุดกระบวนการ · ค้น `AC_VOUCHER` ด้วย `ZZTEST-R13` ได้ 0 แถว |

#### 🔴 บทเรียนโครงสร้าง — node บนสายของ branch ต้องผูกด้วย `prevNode` ไม่ใช่ `parentNode`

v5 สร้าง `rej_dup` / `rej_closed` โดยส่ง **`parentNode: {nodeAlias:"p_isdup"}`** ตามที่ schema บรรยายว่า parentNode ใช้แสดง "ความเป็นเจ้าของในคอนเทนเนอร์ (branch paths)" — **API ตอบ success แต่ผูกผิด**: ทุก node ถูกต่อเรียงกันเป็นสายเดียวตาม `prevNode` ที่ระบบเติมให้เอง (= รายการก่อนหน้าใน array) และ `parentNode` กลายเป็น `null` ทั้งหมด
ผลคือ workflow **ยิงแล้วไม่เขียนอะไรเลย** และ `validate_process` ผ่านฉลุย — หาไม่เจอถ้าไม่อ่าน `get_workflow_structure` กลับมาดู `prevNode` ของทุก node
✅ **วิธีที่ถูก** — node ตัวแรกของแต่ละสายต้องใช้ **`prevNode: {nodeAlias:"<path alias>"}`** (บันทึกเป็น D-42)

---

---

### ผลการไล่ตรวจ workflow ที่ publish แล้วทั้งหมด ตามบทเรียน D-33 / D-34 — 1 ก.ย. 2569

ไล่ `get_workflow_structure` ของ workflow ฝั่งบัญชีที่ publish แล้วทุกตัว มองหาสองอาการ:
**(ก)** `get_multiple` ที่ค้นบนตารางเดียวกับ trigger แต่ไม่ตัดตัวเองออกด้วย `rowid ne trigger`
**(ข)** node ที่เขียนข้อความสถานะ/เหตุผลลงฟิลด์ โดยไม่มีเส้นทางล้างค่า

| Workflow | (ก) | (ข) | สรุป |
|---|---|---|---|
| WF-AC-01 อนุมัติใบสำคัญ | — | 🔴 **เจอ** | `rej_unbal` / `rej_period` เขียน `reject_reason` โดยไม่มีใครล้าง ⇒ **แก้แล้ว publish v3** |
| WF-AC-02 ผ่านรายการเข้า GL | ✅ ไม่มี (ค้น `ac_voucher_line` คนละตารางกับ trigger) | ✅ ไม่มี (ใช้ notification อย่างเดียว) | สะอาด |
| WF-AC-08 กระทบยอดธนาคาร | ✅ | ✅ | สะอาด · trigger เป็น `add` จึงไม่มีปัญหายิงซ้ำ |
| WF-AC-09 ปิดงวด | ✅ | ✅ | สะอาด · `clearFlag1/2` รีเซตธงครบทั้งสองทางที่ยกเลิก |
| WF-AC-11 ปิดปีและยกยอด | ⚠️ **มี แต่ตั้งใจ** | ✅ | `getOpenPeriods` และ `getAllPeriodsThisYear` ค้น `ac_period` ซึ่งเป็นตารางเดียวกับ trigger และ**นับงวดตัวเองด้วย** — ถูกต้องตามความหมาย (จะปิดปีได้ ทุกงวดรวมงวดที่กดต้องปิดก่อน / ล็อกก็ต้องล็อกตัวเองด้วย) ⇒ **ไม่ใช่บั๊ก แต่ต้องรู้ว่าตั้งใจ** |
| WF-AC-12 แจ้งเตือนความผิดปกติ | ✅ | ✅ | สะอาด · schedule trigger · ทุก `get_multiple` อยู่คนละตาราง |
| WF-AC-23 เครื่องคิดภาษี | ✅ | ✅ | สะอาด |
| WF-AC-24 กันใบซ้ำ | ✅ | ✅ (หลังแก้) | ดูหัวข้อของตัวเอง |

#### 🔴 ข้อจำกัดของเครื่องมือที่พบระหว่างไล่ตรวจ — และพิสูจน์แล้วด้วยการทดลอง

**`get_workflow_structure` อ่านเงื่อนไขซ้อนกลุ่มออกมา "แบน" — `A AND (B OR C)` แสดงผลเหมือนกันเป๊ะกับ `A AND B AND C`**

เจอเพราะ gate ของ WF-AC-01 และ WF-AC-02 อ่านออกมาได้เงื่อนไข 4 ข้อต่อกันด้วย `and` โดยมีเงื่อนไขสถานะซ้ำสองครั้ง — ซึ่งถ้าอ่านตามตัวอักษรแปลว่า gate **ไม่มีทางเป็นจริง** แต่ทั้งสองตัวยิงจริงมาตลอด

**การทดลองที่ใช้พิสูจน์ (ทำแล้วลบทิ้ง):** สร้าง workflow ชั่วคราว 1 ตัว ใส่ branch ที่เขียนด้วย `batch_create_process_nodes` เป็น `A AND (B OR C)` จริง ๆ แล้ว
1. อ่านกลับด้วย `get_workflow_structure` → ได้ `and` เงื่อนไข 3 ข้อเรียงกัน (กลุ่ม OR หายไป)
2. publish แล้วยิงกับข้อมูลจริงที่ **B จริง / C เท็จ** → ลงสาขา A **⇒ `PROBE-OR-KEPT`**

⇒ **สรุปสองข้อ**
- ✅ **การเขียน OR ซ้อนผ่าน MCP ทำงานถูกต้อง** — เก็บจริง ใช้จริงตอนรัน (ทางแก้ D-19 ที่เขียนผ่าน MCP จึงเชื่อถือได้)
- 🔴 **ห้ามใช้ `get_workflow_structure` ตรวจตรรกะ branch** — ใครที่ไล่ตรวจทางแก้ D-19 จาก API นี้จะอ่านว่า "พัง" ทั้งที่ถูกอยู่แล้ว และถ้าไป "แก้" ตามที่อ่าน จะทำของดีให้พังจริง · ต้องเปิดดูใน Browser เท่านั้น

#### ✅ ผลทดสอบ WF-AC-01 หลังแก้ (1 ก.ย. 2569)

| เคส | เอกสาร | ผล |
|---|---|---|
| เส้นทางตีกลับยังทำงาน | `ZZTEST-JV-102` (เดบิต 100 / เครดิต 90) | ✅ ตีกลับเป็นร่าง · `reject_reason` = "เดบิตไม่เท่ากับเครดิต ผลต่าง 10.00 บาท …" · `submitted_flag` = 0 |
| **ข้อความเก่าถูกล้างจริง** | `ZZTEST-JV-103` (ดุล 5,000/5,000 · งวด 2569-11 เปิด) — ปลูกข้อความ `STALE-TEST-…` ไว้ก่อน | ✅ `reject_reason` = **ว่าง** · `submitted_flag` = 1 |

> 🆕 **ของแถมจากการทดสอบ: สายอนุมัติของ WF-AC-01 มอบหมายงานได้จริง**
> ใบ `ZZTEST-JV-103` เข้าสายอนุมัติแล้วได้ `_processStatus` = "อนุมัติระดับที่ 1" และ `_nodeAssignees` = ผู้ใช้จริง 1 คน
> ⇒ **สมมติฐานเดิมที่ว่า "ทดสอบสายอนุมัติไม่ได้เพราะ role ยังไม่มีสมาชิก" ไม่จริงสำหรับ WF-AC-01** — ผู้อนุมัติมาจากฟิลด์ OrgRole ของกฎอนุมัติตามวงเงิน ไม่ได้มาจาก app role
> ⚠️ **ผลข้างเคียงที่ต้องเก็บ:** ตอนนี้มีงานอนุมัติค้างอยู่ในกล่อง To-do ของผู้ใช้คนนั้น 1 รายการ (ใบ `ZZTEST-JV-103`) · MCP ไม่มีเครื่องมือยกเลิกใบขออนุมัติ ⇒ **ต้องให้คนกด revoke/ปฏิเสธในหน้าจอ** (`allowRevoke` เปิดอยู่)

---

---

### WF-AC-25 ออกใบสำคัญตั้งหนี้จากใบตั้งหนี้ (งาน 3.5 ครึ่งหลัง) — **สร้างและ publish แล้ว** `6a970bc77dc381d682a550ba` ✅ 2 ก.ย. 2569 · **publishVersion 4** (v3 เพิ่มกันชนงวดปิด · **v4 แก้ relation `journal`/`voucher_type` ที่เขียนเป็นสตริง**) · สายลูก `6a970c20500f6714e479dfe8` (publish v2)

> **ทำไมแยกจาก "สายอนุมัติ"** — งาน 3.5 ในแผนเขียนรวมสองเรื่อง: (1) สายอนุมัติของ AC_AP และ (2) การผ่านรายการ
> สเปกใน `14-FRS-Modules-AC.md` §2 FR-07.1 เขียนเส้นทางไว้ชัดว่า **รออนุมัติ → (Approve node เขียนสถานะ) → รับรู้แล้ว → สร้างใบสำคัญและผ่านรายการ**
> ⇒ ตัดตรงรอยต่อนี้ได้พอดี: **WF-AC-25 รับช่วงตั้งแต่สถานะเป็น "รับรู้แล้ว"** ⇒ ทดสอบได้เต็มรูปแบบโดยไม่ต้องรอสมาชิก role และเมื่อสายอนุมัติสร้างเสร็จ แค่ให้ Approve node เขียนสถานะนี้ก็ต่อกันได้ทันที

- **ชนิด / Surface:** `worksheet_event` (update) บน `AC_AP` · **MCP**
- **Trigger Field = `biz_ap_status` `6a8ec51dae2a0e3743a0b574`** — ตัวเดียวกับ WF-AC-23 / WF-AC-24 (ตัวที่สามบน trigger field เดิม — ตรวจตาม D-35 แล้ว: gate ของสามตัวต้องการสถานะคนละค่า จึงไม่ชนกัน)
- **กันสร้างซ้ำ:** gate ต้องการ `voucher` (ฟิลด์ใบสำคัญที่ตั้งหนี้) **ว่าง** ⇒ ยิงกี่รอบก็ได้ใบเดียว

#### ฟิลด์ที่ต้องเพิ่มก่อน (2 ตัว)

| ฟิลด์ | ID | ทำไมต้องมี |
|---|---|---|
| `AC_AP.biz_ap_tax_invoice_received` (Checkbox) | `6a970b24866e70ecc2e32c2b` | กฎ `AC_POSTING_RULE` แยก `110301 ภาษีซื้อ` กับ `110302 ภาษีซื้อรอเรียกคืน` ด้วยเงื่อนไข "ได้รับใบกำกับแล้วหรือยัง" แต่ **ตารางไม่มีฟิลด์นี้เลย** ⇒ กฎที่เขียนไว้ใช้งานจริงไม่ได้มาตั้งแต่ต้น |
| `AC_VOUCHER.biz_vch_no_auto` (AutoNumber) | `6a970b83526e5edf368cf496` | `voucher_no` เป็น Text ที่ required ⇒ workflow กรอกเลขเองไม่ได้ถ้าไม่มีตัวออกเลข |
...
| # | nodeAlias | ชนิด | ทำอะไร |
|---|---|---|---|
| 1 | `gate` | branch | `p_go`: สถานะ = รับรู้แล้ว `ffca5173-…` **และ** `voucher` ว่าง · `p_skip`: fallback |
| 2 | `get_partner` | get_single | ดึงคู่ค้าเพื่อดูรูปแบบนิติบุคคลและถิ่นที่อยู่ |
| 3 | `get_lines` | get_multiple | บรรทัดค่าใช้จ่ายของใบนี้ · sort `line_no` · ifEmpty `stop` |
| 3.5 | `chk_period` `6a9711c017431051c38acc0e` | get_single | 🆕 **กันชนงวดปิด (R13)** — ค้น `AC_PERIOD` ด้วย `rowid` eq `trigger›biz_ap_period` **และ** `period_status` eq เปิด `f662571c-…` · `ifEmpty` = **stop** ⇒ งวดปิด/ไม่ระบุงวด/หาไม่เจอ = ไม่มีใบสำคัญเกิดขึ้น |
| 4 | `add_vch` | add_record | สร้างใบสำคัญ (สมุดรายวันซื้อ PV · ประเภทเอกสาร AP · งวด/สกุลเงินคัดลอกจากใบตั้งหนี้ · สถานะ ร่าง · `voucher_no` = "(รออกเลข)") |
| 5 | `get_vch` | get_single | อ่านใบสำคัญที่เพิ่งสร้างกลับมา **เพื่อเอาค่า AutoNumber** |
| 6 | `upd_vch_no` | update_record | เขียน `voucher_no` = ค่า AutoNumber |
| 7 | `loop_lines` | **sub_process** | วนบรรทัดค่าใช้จ่ายทีละใบ (สายลูก) |
| 8 | `c_acct` | **code** | ตัดสิน `vatCode` (110301/110302 ตามธงใบกำกับ) · `whtCode` (210303 ต่างประเทศ / 210302 นิติบุคคล / 210301 บุคคลธรรมดา) · `hasVat` · `hasWht` |
| 9–11 | `get_ap_acct` `get_vat_acct` `get_wht_acct` | get_single | ดึง record บัญชีจากผังบัญชี **ด้วยรหัสบัญชีที่ code node คิดได้** |
| 12 | `vl_ap` | add_record | บรรทัดเจ้าหนี้การค้า `210101` เครดิต = ยอดรวมทั้งสิ้น · ระบุคู่ค้า |
| 13 | `b_tax` | branch 4 ทาง | `p_vat_wht` / `p_vat_only` / `p_wht_only` / `p_none` |
| 6.1 | `get_journal` `6a971f197dc381d682a5d042` | get_single | 🆕 v4 — ดึงสมุดรายวัน `PV` จาก `AC_JOURNAL` |
| 6.2 | `get_doctype` `6a971f197dc381d682a5d043` | get_single | 🆕 v4 — ดึงประเภทเอกสาร `AP` จาก `AC_DOC_TYPE` |
| 6.3 | `fix_vch_rel` `6a971f197dc381d682a5d044` | update_record | 🆕 v4 — **เขียน `journal` และ `voucher_type` ใหม่ด้วย field reference** เพราะ `add_vch` เขียนไว้เป็นสตริง `["<rowid>"]` ⇒ กลายเป็น `已删除` (D-44) |
| 14+ | `vl_vat_*` `vl_whtdr_*` `vl_whtcr_*` `link_*` `appr_*` | add_record / update_record | แต่ละสายสร้างบรรทัดที่เหลือ → ผูกใบสำคัญกลับเข้าใบตั้งหนี้ → **ตั้งสถานะใบสำคัญเป็น "อนุมัติแล้ว" ให้ WF-AC-02 รับช่วง** |

**สายลูก** `6a970c20500f6714e479dfe8` — `i_get_vch` (ดึงใบสำคัญแม่จาก `p_voucher_rowid`) → `c_amt` (code) → `i_add_vl`

> 🔑 **ทำไม `c_amt` ต้องเป็น code node** — ยอดเดบิตของบรรทัดค่าใช้จ่ายไม่ใช่ `line_amount` เสมอไป: บรรทัดที่ต้องเสียภาษีต้องใช้ `line_taxable_base` (ซึ่งไม่รวม VAT แม้ตั้งราคาแบบรวมภาษี) ส่วนบรรทัดที่ยกเว้นภาษีต้องใช้ `line_amount` · เขียนเป็นสูตร compute ไม่ได้เพราะเป็นการเลือกตามเงื่อนไข
> 🔑 **ทำไมเลือกบัญชีด้วย code + get_single แทน branch ซ้อน** — ถ้าใช้ branch จะต้องทำ 2 ชั้นแล้วนำโหนดปลายทางมาทำซ้ำทุกสาย (branch ในแพลตฟอร์มนี้ **ไม่มีจุดบรรจบ** — สายที่แยกแล้วไม่กลับมารวมกัน) · การให้ code node คืน "รหัสบัญชี" แล้ว `get_single` ไปหยิบ record มา ทำให้เหลือ branch ชั้นเดียว

#### 🔴 บั๊กที่เจอตอนทดสอบรอบแรก — และเป็นบทเรียนที่ต้องจำ

**เขียนฟิลด์ Relation ด้วย `{kind:"record", node:{...}}` ไม่ได้ — มันเก็บเป็นสตริง `["<nodeId>"]` ลงไปตรง ๆ**

รอบแรกใบสำคัญออกมา **ยอดรวม 0.00 ทั้งเดบิตและเครดิต** ทั้งที่บรรทัดถูกสร้างครบ 6 บรรทัดพร้อมจำนวนเงินถูกต้อง — เพราะฟิลด์ `voucher` และ `account` ของทุกบรรทัดชี้ไปที่ `["6a970be87dc381d682a551d0"]` และแสดงเป็น **`已删除`**
⇒ **อาการเดียวกับ D-shape C ใน `handoff/AC-RELATION-AUDIT.md`** ที่เคยทำให้ `ac_gl.voucher` พังทั้งตาราง

| ใช้ที่ไหน | `kind:"record"` | หมายเหตุ |
|---|---|---|
| `config.target` ของ update_record / delete_record / approval | ✅ **ถูกต้อง จำเป็นต้องใช้** | schema บังคับให้เป็น record |
| `config.fields[].value` ที่เขียนลงฟิลด์ Relation | 🔴 **ห้ามใช้** | ต้องใช้ `{kind:"field", node:X, fieldId:"rowid"}` แทน |

**ยืนยันแล้ว** ว่า `{kind:"field", node:X, fieldId:"rowid"}` ทำงานถูก (เป็นรูปแบบเดียวกับที่ WF-AC-11 ใช้มาตั้งแต่แรกและผ่านการทดสอบ) และการคัดลอก relation → relation ด้วย field reference ก็ถูกเช่นกัน (บรรทัด `account` ที่คัดจาก `AC_AP_LINE.account` ออกมาถูกตั้งแต่รอบแรก)

#### ✅ ผลทดสอบจริง 2 ก.ย. 2569

ใบตั้งหนี้ `ITC-2569/0917` ของ `SUP-0007` (นิติบุคคล · ในประเทศ · ติ๊กว่าได้รับใบกำกับแล้ว) · 2 บรรทัด: ค่าบริการ 30,000 VAT 7% หัก 3% · ค่าขนส่ง 1,000 ยกเว้น VAT หัก 1%

ยอดที่ WF-AC-23 คิดได้: ฐานที่ต้องเสียภาษี 30,000 · ยกเว้น 1,000 · VAT 2,100 · WHT 910 · รวม 33,100 · สุทธิ 32,190

ใบสำคัญ **`PV-2026-00002`** ที่ระบบออกเอง — ดุล 34,010.00 / 34,010.00 · ผ่านรายการแล้ว

| gl_no | gl_id | บัญชี | เดบิต | เครดิต |
|---|---|---|---:|---:|
| GL-000047 | PV-2026-00002-1 | 530204 | 30,000.00 | |
| GL-000050 | PV-2026-00002-2 | 530203 | 1,000.00 | |
| GL-000048 | PV-2026-00002-80 | **110301** ภาษีซื้อ | 2,100.00 | |
| GL-000049 | PV-2026-00002-90 | 210101 | | 33,100.00 |
| GL-000046 | PV-2026-00002-91 | 210101 | 910.00 | |
| GL-000045 | PV-2026-00002-92 | **210302** ภ.ง.ด.53 | | 910.00 |

- ✅ เลือก `110301` ถูกตามธง "ได้รับใบกำกับแล้ว"
- ✅ เลือก `210302` (ภ.ง.ด.53) ถูกจากการอนุมานว่าคู่ค้าเป็นนิติบุคคลในประเทศ
- ✅ ยิงสถานะ "รับรู้แล้ว" ซ้ำอีกครั้ง — **ยังมีใบสำคัญใบเดียว** ไม่สร้างซ้ำ
- ✅ `movement_seq` 45–50 ต่อเนื่องไม่ขาด

#### ⚠️ ความเสี่ยงที่ยังเปิดอยู่

| # | เรื่อง | ทำไมสำคัญ |
|---|---|---|
| R9 | **AutoNumber ออกเป็น `PV-2026-…` ไม่ใช่ `PV-2569-…`** — `createdTime` ให้ปีคริสต์ศักราช | เอกสารบัญชีไทยต้องเป็นพุทธศักราช · แพลตฟอร์มไม่มีตัวเลือกปี พ.ศ. ใน AutoNumber ⇒ ต้องเปลี่ยนไปให้ code node ออกเลขเองถ้าจะให้ตรงระเบียบ |
| R10 | **AutoNumber เดินเลขให้ใบสำคัญ *ทุกชนิด*** ไม่ใช่เฉพาะที่ WF-AC-25 สร้าง | ใบ JV/REV ที่สร้างทางอื่นจะกินเลขไปด้วย ⇒ เลขของ PV จะมีช่องว่าง |
| ~~R11~~ ✅ **ปิดแล้ว 2 ก.ย. 2569** | ~~เส้นทาง `110302` ภาษีซื้อรอเรียกคืน ยังไม่ได้ทดสอบ~~ | ทดสอบจริงแล้วด้วยชุดข้อมูลเสมือน 3 ใบ (`PP-2569/0455` · `RR-2569/0126` · `TTS-2569/1077`) — ดู `18-Seed-Data-AP-Voucher.md` |
| R12 | **ยังไม่ได้สร้างแถวใน `AC_VAT_DOC` และ `AC_WHT`** | ทะเบียนภาษีซื้อและหนังสือรับรองหัก ณ ที่จ่ายยังว่าง — เป็นงาน 4.3 / 5.x |
| ~~R13~~ ✅ **ปิดแล้ว 2 ก.ย. 2569** | ~~ไม่มีการตรวจว่างวดบัญชียังเปิดอยู่ก่อนออกใบสำคัญ~~ | ปิดสองชั้น: **(1)** ด่าน `b_check` ใน **WF-AC-24** ตีกลับตั้งแต่ตอนส่งอนุมัติพร้อมเหตุผล BR-03 · **(2)** `chk_period` (`ifEmpty:stop`) ใน WF-AC-25 กันกรณีมีใครเขียนสถานะ "รับรู้แล้ว" ตรง ๆ ข้ามด่าน · ยืนยันด้วย TC-R13-a/b/c |
| R14 🆕🔴 | **ยิงพร้อมกันหลายใบแล้ว workflow ล้มเงียบ — เอกสารหายไปทั้งใบ** | ทดสอบ seed 30 ใบ: ยิง `รับรู้แล้ว` พร้อมกัน 5 ใบได้ใบสำคัญ 4 · พร้อมกัน 3 ใบตกอีกหลายรอบ · **รวมตก 10 จาก 30** · ใบที่ตกไม่มีร่องรอยเลย แต่ **เลข AutoNumber ถูกกินไปแล้ว** ⇒ `add_vch` สร้างแถวสำเร็จแล้วกระบวนการถอยคืน · ผู้ต้องสงสัยหลักคือ `get_vch` ที่อ่านแถวที่เพิ่งสร้างกลับมาทันทีแบบ `ifEmpty:stop` · **แก้ชั่วคราวด้วยการยิงทีละใบเว้น ~20 วินาที (สำเร็จ 10/10)** · **ข้อเสนอการแก้:** เปลี่ยน `get_vch.ifEmpty` เป็น `continue` และให้ทุก node ที่อ้าง `get_vch›rowid` ไปอ้าง `add_vch›rowid` แทน ⇒ กรณีแข่งกันจะเสียแค่ `voucher_no` (เห็นได้ชัดว่ายังเป็น "(รอออกเลข)") ไม่ใช่เสียทั้งเอกสาร · **ต้องแก้ก่อนใช้จริง** (บันทึกเป็น D-45) |
| ~~R15~~ ✅ **พบและแก้แล้ว 2 ก.ย. 2569** | ~~`journal` และ `voucher_type` ของใบสำคัญทุกใบเป็น `已删除`~~ | `add_vch` เขียน Relation ด้วย `{kind:"literal", value:"[\"<rowid>\"]"}` มาตั้งแต่ v1 และ WF-AC-02 คัดลอกค่าเสียต่อไปที่ `AC_GL.journal` ทุกแถว · แก้ด้วย `get_journal`/`get_doctype`/`fix_vch_rel` (v4) และซ่อมข้อมูลเดิม: ใบสำคัญ 31 ใบ + GL 135 แถว (บันทึกเป็น D-44) |
