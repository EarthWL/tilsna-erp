...
| # | เรื่อง | ทำไมสำคัญ |
|---|---|---|
| R1 | **`non_taxable_base` ของใบที่ไม่มีบรรทัดยกเว้นเลย ได้ค่า *ว่าง* ไม่ใช่ 0** (เห็นในใบ GROSSUP-003) — rollup ที่ไม่เจอแถวคืนค่าว่าง | ยอดรวมยังถูกเพราะ `compute` ตั้ง `nullZero` ไว้ แต่ **ฟิลด์ที่ปล่อยว่างจะไประเบิดกับ filter `= 0` ปลายน้ำ** ซึ่งเป็นบทเรียน D-19 เดิม ⇒ ควรบังคับเป็น 0 |
