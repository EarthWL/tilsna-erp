...
_อัปเดตล่าสุด: 28 ส.ค. 2569 (รอบดึกที่สุด — ทำความสะอาด workflow object ซ้ำซ้อน/orphan ทั้งแอป) — **ลบ orphan/duplicate workflow object ที่ค้างจากรอบ build/test ก่อนหน้า**: WF-AC-01 orphan approval-flow child 2 ตัว + WF-AC-02 stale human-authored draft 1 ตัว + WF-AC-10 orphan sub_process 7 ตัว + ปุ่ม Custom Action ซ้ำ "ปิดปีและยกยอด" บน AC_PERIOD 1 ตัว (test draft เก่า) — ยืนยันด้วย `hap workflow list` (CLI) cross-reference กับ `get_workflow_structure` (MCP) ก่อนลบทุกครั้ง · 🆕 **ค้นพบว่า CLI `hap workflow list <appId> [-k keyword] -n <n>` เป็นเครื่องมือ audit workflow ทั้งแอปที่ครอบคลุมที่สุด** — เห็น process ทุกตัวรวม draft/disabled/orphan ที่ MCP `get_workflow_list` (เห็นแค่ PBP-type) มองไม่เห็นเลย ⇒ ใช้เป็นเครื่องมือ audit มาตรฐานจากนี้ไป · เพิ่ม**กับดักข้อ 35**: อธิบายแพทเทิร์น "build-iteration ทิ้ง orphan duplicate process ไว้เสมอ" (เพราะ `approval_block`/`sub_process` แก้ในตัวไม่ได้ ตามกับดักข้อ 24) + เทคนิคใหม่ว่าปุ่ม Custom Action ที่ชื่อซ้ำกัน ลำดับแถวบนหน้าจอไม่เสถียร ต้องยืนยันตัวตนด้วย Button Description + network request เท่านั้น ห้ามใช้ตำแหน่งแถว · พบความไม่เรียบร้อยแบบเดียวกันในสโคป `tilsna-hr` (attendance-sync helper ×3, WF-HR-05 inner helper ซ้ำ) แต่จงใจไม่แตะ นอกสโคปโมดูลบัญชี — ดูกับดักข้อ 35 และแถว #39 ด้านล่างสำหรับรายละเอียดเต็ม (รอบก่อนหน้า: 28 ส.ค. 2569 (รอบเช้า — ยืนยันข้อสมมติ A-11 ปลดบล็อก FR-13/Phase 7) — **ผู้ใช้ยืนยัน (ภาษาไทย): "ยืนยัน บันทึกข้อมูลนี้เข้าเอกสารไว้"** ⇒ **บัญชีถือสมุดบัญชีสินทรัพย์ (`สมุดบัญชีสินทรัพย์`) เต็มรูปแบบ**: ราคาทุน อายุการใช้งาน มูลค่าซาก วิธีคิดค่าเสื่อม/ตารางค่าเสื่อม ค่าเสื่อมสะสม และกำไร/ขาดทุนจากการตัดจำหน่าย — **FR-13 อยู่ในขอบเขตเต็มรูปแบบและปลดบล็อกแล้ว** (section `AC-06 Fixed Assets` ยังว่างเปล่า ยังไม่ได้ลงมือสร้าง — รอบนี้เป็นแค่การบันทึกการยืนยัน ไม่มีการ build ใด ๆ) โมดูลสินทรัพย์กายภาพ (ถ้าสร้างในอนาคต) จะถือเฉพาะการครอบครอง ตำแหน่ง และการตรวจนับ เชื่อมกันด้วยรหัสสินทรัพย์ร่วม ตรงกับ `01-BRD.md` §4.2 · sync ครบทุกเอกสารแล้ว (`01-BRD.md` §7/§10/§11, `02-BuildSpec-FRS.md` AC-06, `03-RTM-Status.md` BR-12, `05-Roadmap-Tracker.md` Phase 7/§3.2/§5, `06-Demo-Plan-Checklist.md` §2) — build agent ในอนาคตดำเนินการ FR-13 ต่อได้ทันทีที่มีการจัดลำดับความสำคัญ (รอบก่อนหน้า: อัปเดตล่าสุด: 28 ส.ค. 2569 (รอบดึกกว่าเดิม — ยืนยันข้อสมมติ A-09 ปิด FR-12/Phase 6 ถาวร) — **ผู้ใช้ยืนยัน (ภาษาไทย): "จะทำโมดูลการขายเข้ามาเพิ่มภายหลัง ให้เซลล์เป็นคนออก" ⇒ บัญชีไม่ได้เป็นเจ้าของห่วงโซ่เอกสารขาย (FR-12) อีกต่อไป — ย้ายออกนอกขอบเขตถาวร** จะมีโมดูลการขายแยกต่างหากสร้างในอนาคต ฝ่ายขายเป็นผู้ออกใบแจ้งหนี้/ใบกำกับภาษี/ใบเสร็จ/ใบลดหนี้/ใบเพิ่มหนี้เอง — ตรงกับผลที่ `01-BRD.md` §11 A-09 คาดการณ์ไว้แต่แรกว่า "ถ้ามีระบบขายแยกต่างหาก ขอบเขตนี้ย้ายออกทั้งก้อน" ⇒ **ห้าม build agent สร้าง worksheet `AC_INV`/`AC_BL`/`AC_CN`/`AC_DN`/`AC_AR`/`AC_AR_LINE`/`AC_RE` (section `AC-05`) เด็ดขาด** — ทั้งหมดยังเป็น `<TBD>` ไม่เคยสร้างจริง จึงไม่มี data/object ให้ต้อง cleanup · จุดเชื่อมต่อในอนาคตของบัญชีคือ**รับ**รายการรายได้/ลูกหนี้ที่ผ่านรายการแล้วจากโมดูลขาย (รูปแบบเดียวกับ Procurement/HRMS) ไม่ใช่ออกเอกสารเอง · sync ครบทุกเอกสารแล้ว (`01-BRD.md` §4.1/§7/§8/§10/§11/§12, `02-BuildSpec-FRS.md` FR-12, `03-RTM-Status.md` BR-11, `05-Roadmap-Tracker.md` Phase 6/§3.2/§5, `06-Demo-Plan-Checklist.md` §4) · Phase 6 (5 งาน 6.1–6.5) ย้ายออกนอกขอบเขตถาวรใน Roadmap — ตัวเลขภาพรวม 86 งานยังไม่ปรับ รอผู้ใช้ตัดสินใจ (รอบก่อนหน้า: อัปเดตล่าสุด: 28 ส.ค. 2569 (รอบดึก — เพิ่มความสามารถ "ตั้งค่าได้" ให้ข้อสมมติ A-08 + พบกับดักใหม่ข้อ 34) — **สร้างตารางตั้งค่าใหม่ "พารามิเตอร์ระบบบัญชี" (`AC_SYSTEM_PARAM`, ws `6a917a2d9762533b5b720392`) ใต้กลุ่ม AC-00 ตามคำขอผู้ใช้ให้ทำให้จุดตัดข้อมูล/วันขึ้นระบบ (BRD A-08) "ตั้งค่าได้"** — สร้างผ่าน workaround เดิม (กับดักข้อ 25) มี 4 ฟิลด์ (`biz_setting_name` ชื่อพารามิเตอร์ · `biz_cutover_date` วันตัดยอดข้อมูล · `biz_go_live_date` วันขึ้นระบบจริง · `biz_note` หมายเหตุ) + 1 record เริ่มต้น (`biz_setting_name`="พารามิเตอร์หลักของระบบบัญชี" ค่าวันที่ทั้งสองยังว่าง รอกรอกจริง) · view เริ่มต้นเปลี่ยนชื่อเป็น `ทั้งหมด` แล้วผ่าน CLI `hap` (ตามแพทเทิร์นกับดักข้อ 33) · **สิ่งนี้เพิ่มแค่ "ความสามารถ" ให้ตั้งค่าได้ในอนาคต ไม่ได้แปลว่า A-08 ถูกยืนยันหรือมีค่าจริงแล้ว** — สถานะ A-08 ยังคง 🔴 ยังไม่กำหนด ต้องรอผู้รับผิดชอบกรอกวันที่จริงก่อนเริ่ม Phase 12 เหมือนเดิม (ดู `05-Roadmap-Tracker.md` §3.2 และ `01-BRD.md` §11) · ระหว่างทางพบว่า worksheet-level `alias` (ตั้งผ่าน `update_worksheet.alias`) ไม่ persist จริง และไม่มี MCP/CLI tool ใดยืนยันค่าจริงได้เลย (field-level alias `biz_*` ทำงานปกติ ไม่กระทบ formula/filter/workflow) — เพิ่มเป็น**กับดักข้อ 34** · role permission ของตารางใหม่นี้ยังไม่ได้ตั้ง — รอทำผ่าน Browser ตาม Build Spec §1.4 (ธรรมเนียมเดิมของกลุ่ม AC-00) (รอบก่อนหน้า: 28 ส.ค. 2569 (รอบค่ำ — แก้ไข claim update_view + ปิด view gap AC-07 + ปิด loop HR payroll COA เต็ม) — **แก้ไขคำกล่าวอ้างเดิมว่า "ไม่มี update_view ใน API"**: MCP connector ยังไม่มี tool `update_view` จริง (กับดักข้อ 11 ยังถูกต้องสำหรับ MCP) แต่พบว่า **CLI `hap` แยกต่างหาก** (คนละเครื่องมือกับ MCP เรียกผ่าน Bash โดยตรง) มีคำสั่ง `hap worksheet view update <worksheet_id> <view_id> --name "..."` ใช้งานได้จริง ยิงสำเร็จ 5 ครั้งเปลี่ยนชื่อ view จาก `全部` เป็น `ทั้งหมด` บน 5 ตารางของกลุ่ม AC-07 ที่สร้างขึ้นหลังรอบตรวจสอบ view เดิม 26 ส.ค. 2569 (การกระทบยอดธนาคาร, รายการกระทบยอดธนาคาร, รายการตรวจสอบปิดงวด, การปิดบัญชีสิ้นปี/AC_CLOSING_ENTRY, ยอดยกมาต้นปี/AC_OPENING) — ยืนยันด้วย JSON readback ของ CLI เองว่า `name` เปลี่ยนจริงครบทั้ง 5 ⇒ **รวม view เป็นไทยทั้งระบบจาก 62/62 (26 ส.ค.) เป็น 67/67** — เพิ่มเป็นกับดักข้อ 33 ด้านล่าง · **ปิด loop การช่วยเหลือ HR**: `hr_pay_component.biz_coa_account` เชื่อมครบ **6/6** แล้ว (`SSO_ER`/`SSO_EE`/`WHT` เชื่อมเพิ่มในรอบนี้ผ่าน `update_record` ต่อจาก `SALARY`/`OT`/`ABSENCE_DEDUCT` ที่มีอยู่ก่อนหน้า) — ดูหัวข้อ "ช่วยเหลือ HR — ปิด Gap ผังบัญชีเงินเดือน" ด้านล่าง · **🆕 [28 ส.ค. 2569 ดึกกว่ารอบค่ำ] สแกนทั้งแอปหา view `全部`/`Main` ที่อาจหลงเหลือเสร็จสมบูรณ์แล้ว**: ตรวจครบทั้ง **68 worksheet / 102 view** ในแอปนี้ (ไม่ใช่แค่ 39 ตารางในสโคปโมดูลบัญชี — รวมทุกตารางของทั้งแอปที่ MCP connector มองเห็น) ผ่าน CLI `hap worksheet list` + `hap worksheet view list` วนทุกตาราง แล้วตรวจด้วย regex CJK (`[一-鿿]`) กับชื่อ view ทุกแถว **ผลลัพธ์: 0 view ที่มีอักษรจีนหรือใช้ค่าเริ่มต้น `Main` หลงเหลืออยู่เลย** — ปิด follow-up "ยังไม่ได้สแกนทั้งแอป" ที่ค้างจากรอบก่อนหน้าอย่างถาวร ไม่ต้องสแกนซ้ำอีกจนกว่าจะมีตารางใหม่เพิ่มในอนาคต (รอบก่อนหน้า: 28 ส.ค. 2569 (รอบเย็น ต่อ — ช่วย HR ปิด Gap G-02) — **ช่วยฝ่าย HR ปิด Gap ผังบัญชีเงินเดือน**: สร้างบัญชีใหม่ 3 รายการใน AC_COA (`210304` ภาษีหัก ณ ที่จ่ายค้างจ่าย-ภ.ง.ด.1(เงินเดือน), `210402` เงินสมทบประกันสังคมค้างนำส่ง(ลูกจ้าง), `530102` เงินสมทบประกันสังคม(นายจ้าง)) ผ่าน `create_record`(`triggerWorkflow:false`) ยืนยันครบทั้ง 3 ด้วย `get_record_details` (ทุก Dropdown field ลงถูก key ไม่มี option ซ้ำ) · เพิ่ม 2 option ใหม่ `PAYROLL_ACCRUAL`/`PAYROLL_PAYMENT` เข้า `ac_posting_rule.event_code` ผ่าน `update_worksheet.editFields` สำเร็จ ปิด **Gap G-02 ของ HR** · **แก้ข้อมูลที่ HR เข้าใจผิด**: `event_code` เป็นฟิลด์ inline SingleSelect ธรรมดา ไม่ใช่ shared optionset อย่างที่ HR สันนิษฐาน (ไม่ต้องพึ่ง Browser) · **พบกับดักใหม่ — เพิ่มเป็นกับดักข้อ 32**: `editFields` แก้ field ที่มีอยู่โดยไม่ส่ง `name`/`alias` กลับไป ทำให้ทั้งสองค่าถูกล้างเป็นค่าว่างเงียบ ๆ ทั้งที่ไม่ได้ตั้งใจแตะ — จับได้จาก `get_worksheet_structure` ทันทีหลังแก้ แล้วซ่อมด้วยการเรียกซ้ำพร้อมส่ง `name`+`alias` กลับไป · ยืนยัน/ทำให้กับดักข้อ 12 ชัดเจนขึ้นด้วยหลักฐานใหม่: การส่ง `options` ที่ reproduce ค่าตัวเลือกเดิมครบทุกตัวตรงข้อความ/ลำดับ จะไม่เกิดคีย์ซ้ำ (คงคีย์เดิม มินต์คีย์ใหม่เฉพาะตัวเลือกที่เพิ่มจริง — 12 option สุดท้ายไม่ซ้ำเลย) · endorsed การตัดสินใจ source-of-truth ของ WF-HR-10 (`ac_posting_rule` เป็นหลัก, `hr_pay_component.biz_coa_account` เป็น fallback) — รายละเอียดเต็มดูหัวข้อ "ช่วยเหลือ HR — ปิด Gap ผังบัญชีเงินเดือน" ด้านล่าง (รอบก่อนหน้า: 28 ส.ค. 2569 — **งาน 8.5 WF-AC-11 ปิดปีและยกยอด สร้าง publish (v1 `6a90ef69fdab77a41c5b514f`) และทดสอบผ่านครบทั้ง 2 เส้นทาง**: 56 nodes + trigger + 4 inner sub-process บน `AC_PERIOD` ผ่านฟิลด์ใหม่ `biz_close_year_flag` + ปุ่ม Custom Action "ปิดปีและยกยอด" · **ออกแบบใหม่จาก spec เดิม 3 จุด**: (1) `AC_GL.account` เป็น Relation ไม่มี Lookup ให้ filter ตาม `account_type` ผ่าน API ⇒ hardcode รายการ rowid บัญชีรายได้ 4 + ค่าใช้จ่าย 12 ตัวตรง ๆ (2) ยืนยันว่าบั๊ก `Condition.op:"in"` (กับดักข้อ 28) ครอบคลุมถึง **Relation field** ด้วย ไม่ใช่แค่ Dropdown/SingleSelect ⇒ แตกเป็น 1 rollup ต่อ 1 บัญชี (รวม 32 rollup) (3) `source_module` ไม่มีตัวเลือก "CLOSING" จริง ⇒ ใช้คีย์ "ปิดงวด" เดิมแทน — เพิ่มเป็น**กับดักข้อ 31** พร้อมยืนยันเพิ่ม 2 ข้อ (rollup sum/avg/min/max ไม่ต้องมี `config.target`, `sub_process` ใช้ `sub_trigger` filter แบบไดนามิกต่อ record ที่ iterate ได้จริง) · AC_OPENING เก็บเดบิต/เครดิตรวมแบบ gross สะสมถึงปีที่ปิด (ไม่ netted) เป็นการออกแบบตั้งใจ · ทดสอบผ่านทั้งเส้นทางปฏิเสธและเส้นทางสำเร็จจริง: รายได้ 165,000.00 / ค่าใช้จ่าย 52,500.00 / กำไรสุทธิ 112,500.00 ตรงกับคำนวณมือทุกบาท, สร้าง AC_OPENING 27 รายการ (Asset 16, Liability 9, Equity 2), ล็อกครบ 13 งวด · ⚠️ Gap: ไม่มีการสร้าง AC_VOUCHER_LINE คู่เดบิต/เครดิตอัตโนมัติ (นอกขอบเขต) · ปุ่มยังไม่ยืนยัน Scope ผ่าน Browser · Phase 8 จาก 67%→**83%** (✅5/6) รวมทั้งโปรเจกต์ 37%→**38%** (✅33/86) — เอกสารเดิมต่อจากนี้ดูรายละเอียดฉบับเต็มในไฟล์นี้ด้านล่าง))))))_
...
| # | งาน | สถานะ |
|---|---|---|
| 1–35 | (ดูรายละเอียดเต็มใน `05-Roadmap-Tracker.md` §2 — ทุกแถวตรงกัน sync แล้ว) | ✅ ตามที่ระบุใน Roadmap Tracker |
| 36 | **ช่วยฝ่าย HR ปิด Gap G-02 (ผังบัญชีเงินเดือน) — สร้างบัญชี 3 รายการ + option event_code 2 ตัว + แก้ข้อเข้าใจผิดของ HR** | ✅ 28 ส.ค. 2569 (รอบบ่าย/เย็น) |
| 37 | **[28 ส.ค. 2569 รอบค่ำ] แก้ไข claim "ไม่มี update_view" + ปิด view gap 5 ตาราง AC-07 ผ่าน CLI `hap` + เชื่อม `hr_pay_component.biz_coa_account` ครบ 6/6 + สแกนทั้งแอปยืนยัน 0 view จีน/`Main` หลงเหลือ** | ✅ — CLI `hap worksheet view update` ยิงสำเร็จ 5/5 ตาราง (67/67 view เป็นไทยครบ) · เชื่อม COA ให้ `WHT`/`SSO_EE`/`SSO_ER` ผ่าน `update_record` ยืนยันครบ 6/6 · สแกนครบทั้งแอป 68 worksheet/102 view ยืนยัน 0 view หลงเหลือ · ไม่กระทบตัวเลขภาพรวม 86 งานของโปรเจกต์บัญชี (งานนี้เป็นงานเสริมด้านเอกสาร/ข้ามโปรเจกต์ ไม่ใช่ task ในรายการ 86 งาน) |
| 38 | **[28 ส.ค. 2569 รอบดึก] สร้างตารางตั้งค่า AC_SYSTEM_PARAM (พารามิเตอร์ระบบบัญชี) ตอบสนองคำขอทำให้ BRD A-08 "ตั้งค่าได้" + พบกับดักใหม่ #34 (worksheet-level alias ไม่ persist)** | ✅ (เพิ่มความสามารถ — **ไม่ใช่การปิด A-08**, A-08 ยังคง 🔴) — ws `6a917a2d9762533b5b720392`, 4 ฟิลด์ `biz_setting_name`/`biz_cutover_date`/`biz_go_live_date`/`biz_note`, 1 record เริ่มต้น (`65eb8bc0-ac6b-4301-96a8-9e3b0bf2ba89`, ค่าวันที่ว่าง), view `ทั้งหมด` ผ่าน CLI, role permission ยังไม่ได้ตั้ง (pending manual) |
| 39 | **[28 ส.ค. 2569 รอบดึกที่สุด] Audit ทั้งแอปหา workflow ซ้ำซ้อน/orphan ผ่าน CLI `hap workflow list` + ลบ 2+1+7 orphan process (WF-AC-01/WF-AC-02/WF-AC-10) + ลบปุ่ม Custom Action ซ้ำ 1 ตัว (WF-AC-11 AC_PERIOD) + เพิ่มกับดักข้อ 35** | ✅ — ลบ orphan approval-flow child ของ WF-AC-01 2 ตัว (ยืนยันด้วย `triggerId`), ลบ stale human draft ของ WF-AC-02 1 ตัว (`6a861cfef520ee95fb0c3a3a`), ลบ orphan `sub_process` ของ WF-AC-10 7 ตัว (5× reverseLines, 1× markGLReversed, 1× reverseVatDocs) — cross-reference กับ `get_workflow_structure` ก่อนลบทุกครั้ง, ลบปุ่ม Custom Action ซ้ำ "ปิดปีและยกยอด" อันเก่า (triggerId `6a90ef438b6633ef76f16f5a`) ผ่าน Browser · ค้นพบเทคนิค audit ใหม่: CLI `hap workflow list` เห็น process ทุกตัวรวม orphan ที่ MCP `get_workflow_list` (PBP-only) มองไม่เห็น — บันทึกเป็นกับดักข้อ 35 · พบความไม่เรียบร้อยแบบเดียวกันในสโคป `tilsna-hr` (attendance-sync helper ×3, WF-HR-05 inner helper ซ้ำ) แต่จงใจไม่แตะ — อยู่นอกสโคปโมดูลบัญชี |
